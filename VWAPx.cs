...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private VWAPx[] cacheVWAPx;
		public VWAPx VWAPx()
		{
			return VWAPx(Input);
		}

		public VWAPx VWAPx(ISeries<double> input)
		{
			if (cacheVWAPx != null)
				for (int idx = 0; idx < cacheVWAPx.Length; idx++)
					if (cacheVWAPx[idx] != null &&  cacheVWAPx[idx].EqualsInput(input))
						return cacheVWAPx[idx];
			return CacheIndicator<VWAPx>(new VWAPx(), input, ref cacheVWAPx);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.VWAPx VWAPx()
		{
			return indicator.VWAPx(Input);
		}

		public Indicators.VWAPx VWAPx(ISeries<double> input )
		{
			return indicator.VWAPx(input);
		}
	}
